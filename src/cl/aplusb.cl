...
__kernel void aplusb(__global const float* a, __global const float* b, __global float* c, unsigned int num)
{
    size_t wi_id = get_global_id(0);

    if (wi_id > num) {
        return;
    }
    c[wi_id] = a[wi_id] + b[wi_id];

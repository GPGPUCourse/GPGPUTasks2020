...
__kernel void radix_bits(__global unsigned int* as, __global int* zero_positions, __global int* one_positions, unsigned int nth_bit, unsigned int n)
{
    int global_id = get_global_id(0);
    if (global_id < n) {
        ...
        if (bit_value) {
            one_positions[global_id] = 1;
            zero_positions[global_id] = 0;
        } else {
            zero_positions[global_id] = 1;
            one_positions[global_id] = 0;
        }
    }
}

__kernel void radix_set(__global unsigned int* as_old, __global unsigned int* as_new, __global int* zero_positions, __global int* one_positions,
                    unsigned int nth_bit, unsigned int n)
{
    ...
    if (global_id < n) {
        ...
        int bit_value = 1 == ((number >> nth_bit) & 1);
        if (bit_value) {
            as_new[one_positions[global_id] + zero_positions[n - 1] - 1] = number;
        } else {
            as_new[zero_positions[global_id] - 1] = number;
        }
    }
}

...
int main(int argc, char **argv)
{
...

    {
        ...
        {
            ocl::Kernel gpu_sum(sum_kernel, sum_kernel_length, "simple_sum");
            gpu_sum.compile();
            timer t;
            for (int i = 0; i < benchmarkingIters; ++i) {
                unsigned int sum = 0;
                sum_gpu.writeN(&sum, 1);
                gpu_sum.exec(gpu::WorkSize(workGroupSize, global_work_size),
                             as_gpu, n, sum_gpu);
                sum_gpu.readN(&sum, 1);
        ...
        {
            ocl::Kernel gpu_sum(sum_kernel, sum_kernel_length, "tree_sum");
            gpu_sum.compile();
            timer t;
            unsigned int values_per_item = 64;
            global_work_size = (n + values_per_item - 1) / values_per_item;
            for (int i = 0; i < benchmarkingIters; ++i) {
                unsigned int sum = 0;
                sum_gpu.writeN(&sum, 1);
                gpu_sum.exec(gpu::WorkSize(workGroupSize, global_work_size),

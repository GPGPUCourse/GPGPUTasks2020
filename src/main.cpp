...
int main() {
    ...
    for (int platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
        ...
        for (int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
            ...
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(cl_ulong), &deviceGlobalMemSize,
                                          nullptr));
            std::cout << "    Global Mem Size: " << deviceGlobalMemSize / 1024 / 1024 << " MB" << std::endl;

            cl_bool deviceImageSupport = CL_FALSE;
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_IMAGE_SUPPORT, sizeof(cl_bool), &deviceImageSupport,
                                          nullptr));
            std::cout << "    Device Image Support: " << (deviceImageSupport == CL_TRUE ? "Yes" : "No") << std::endl;

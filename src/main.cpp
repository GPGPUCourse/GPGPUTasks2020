...
{    
    ...
    for (int platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
        cl_platform_id currentPlatform = platforms[platformIndex];
        cl_uint devicesCount = 0;
        OCL_SAFE_CALL(clGetDeviceIDs(currentPlatform, CL_DEVICE_TYPE_ALL, 0, nullptr, &devicesCount));
        std::vector<cl_device_id> devices(devicesCount);
        OCL_SAFE_CALL(clGetDeviceIDs(currentPlatform, CL_DEVICE_TYPE_ALL, devicesCount, devices.data(), nullptr));
        for (int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
            cl_device_id currentDevice = devices[deviceIndex];
            cl_device_type currentDeviceType = 0;
            OCL_SAFE_CALL(clGetDeviceInfo(currentDevice, CL_DEVICE_TYPE, sizeof(cl_device_type), &currentDeviceType,
                                          nullptr));
            bool currentIsCPU = currentDeviceType >> 1u;
            bool currentIsGPU = currentDeviceType >> 2u;
            if (!isGPU && (currentIsGPU || currentIsCPU)) {
                isGPU = currentIsGPU;
    ...
    if (!deviceId) {
        throw std::runtime_error("Unable to find CPU or GPU device!");
    }
    std::cout << "Device name: " << getDeviceStringParam(deviceId, CL_DEVICE_NAME) << std::endl;
    ...
    size_t const bufSize = sizeof(float) * n;
    cl_mem bufA = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, bufSize, as.data(), &errCode);
    OCL_SAFE_CALL(errCode);
    cl_mem bufB = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, bufSize, bs.data(), &errCode);
    OCL_SAFE_CALL(errCode);
    cl_mem bufC = clCreateBuffer(context, CL_MEM_WRITE_ONLY, bufSize, nullptr, &errCode);
    OCL_SAFE_CALL(errCode);
    ...
    const char *programSrcPtr = kernel_sources.c_str();
    const size_t lengths[] = {0};
    cl_program program = clCreateProgramWithSource(context, 1, &programSrcPtr, lengths, &errCode);
    OCL_SAFE_CALL(errCode);
    ...
    {
        unsigned int i = 0;
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, bufSize, &bufA));
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, bufSize, &bufB));
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, bufSize, &bufC));
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        std::cout << "GFlops: " << float(n) / t.lapAvg() * 1e-9 << std::endl;
        ...
        // - Среднее время выполнения кернела равно t.lapAvg() секунд
        std::cout << "VRAM bandwidth: " << float(3*n*sizeof(float)) / 1024 / 1024 / 1024 / t.lapAvg()  << " GB/s" << std::endl;
    }
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            // clEnqueueReadBuffer...
            OCL_SAFE_CALL(clEnqueueReadBuffer(commandQueue, bufC, CL_TRUE, 0, bufSize, cs.data(), 0, nullptr, nullptr));
            t.nextLap();
        }
    ...
    OCL_SAFE_CALL(clReleaseMemObject(bufA));
    OCL_SAFE_CALL(clReleaseMemObject(bufB));
    OCL_SAFE_CALL(clReleaseMemObject(bufC));

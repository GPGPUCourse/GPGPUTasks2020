...
cl_device_id getBestDevice() {
	...
	cl_device_id bestCPU = nullptr;
	cl_device_id bestGPU = nullptr;
...
{    
	...
	OCL_SAFE_CALL(clGetCommandQueueInfo(commandQueue, CL_QUEUE_PROPERTIES, sizeof(queueProperties), &queueProperties, nullptr));
	
	if (queueProperties & CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE)
		throw std::runtime_error("Command queue created in out of order mode");
	...
	cl_mem aBuffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, as.data(), &errcode);
	OCL_SAFE_CALL(errcode);
	
	cl_mem bBuffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, bs.data(), &errcode);
	OCL_SAFE_CALL(errcode);
	
	cl_mem cBuffer = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(float) * n, nullptr, &errcode);
	OCL_SAFE_CALL(errcode);
	...
	cl_program program = clCreateProgramWithSource(context, 1, &src, nullptr, &errcode);
	OCL_SAFE_CALL(errcode);
	...
	// TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
	{
		...
		for (unsigned int i = 0; i < 20; ++i) {
			cl_event event;
			OCL_SAFE_CALL(clEnqueueReadBuffer(commandQueue, cBuffer, CL_FALSE, 0, sizeof(float) * n, cs.data(), 0, nullptr, &event));
			OCL_SAFE_CALL(clWaitForEvents(1, &event));
			t.nextLap();
		}

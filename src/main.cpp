...
size_t RoundUp(size_t n, size_t group_size) {
    return (n + group_size - 1) / group_size * group_size;
}

void main_for_device(cl_platform_id platform, cl_device_id device, size_t n) {
    constexpr size_t work_group_size = 128;
    {
    ...
    std::vector<float> as(global_work_size, 0.0);
    std::vector<float> bs(global_work_size, 0.0);
    std::vector<float> cs(global_work_size, 0.0);
    FastRandom r(n);
    ...
    ClBuffer cs_gpu(*context, CL_MEM_WRITE_ONLY, sizeof(float) * global_work_size, nullptr);
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            OCL_SAFE_CALL(
                    clEnqueueWriteBuffer(*queue, *as_gpu, CL_TRUE, 0, sizeof(float) * global_work_size, as.data(), 0,
                                         nullptr, nullptr));
            OCL_SAFE_CALL(
                    clEnqueueWriteBuffer(*queue, *bs_gpu, CL_TRUE, 0, sizeof(float) * global_work_size, bs.data(), 0,
                                         nullptr, nullptr));
            OCL_SAFE_CALL(
                    clEnqueueWriteBuffer(*queue, *cs_gpu, CL_TRUE, 0, sizeof(float) * global_work_size, cs.data(), 0,
                                         nullptr, nullptr));
            t.nextLap();
        }
        auto avg_time = t.lapAvg();
        std::cout << "Result data transfer time: " << avg_time << "+-" << t.lapStd() << " s\n"
                  << "RAM -> VRAM bandwidth: " << 3 * global_work_size * sizeof(float) / avg_time / (1024 * 1024 * 1024)
                  << " GB/s" << std::endl;
    }
    ...
    {
        unsigned int i = 0;
        clSetKernelArg(*kernel, i++, sizeof(cl_mem), (void *) as_gpu.AsPtr());
        clSetKernelArg(*kernel, i++, sizeof(cl_mem), (void *) bs_gpu.AsPtr());
        clSetKernelArg(*kernel, i++, sizeof(cl_mem), (void *) cs_gpu.AsPtr());
    }
    {
        ...
        std::cout << "GFlops: " << global_work_size / 1e9 / avg_time << std::endl;
        std::cout << "VRAM bandwidth: " << 3 * global_work_size * sizeof(float) / avg_time / (1024 * 1024 * 1024)
                  << " GB/s" << std::endl;
    }

    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            OCL_SAFE_CALL(
                    clEnqueueReadBuffer(*queue, *cs_gpu, CL_TRUE, 0, sizeof(float) * global_work_size, cs.data(), 0,
                                        nullptr, nullptr));
            t.nextLap();
        }
        auto avg_time = t.lapAvg();
        std::cout << "Result data transfer time: " << avg_time << "+-" << t.lapStd() << " s\n"
                  << "VRAM -> RAM bandwidth: " << global_work_size * sizeof(float) / avg_time / (1024 * 1024 * 1024)
                  << " GB/s" << std::endl;
    }
    ...
    for (unsigned int i = 0; i < global_work_size; ++i) {
        ...
        if (std::abs(cs[i] - sum) > std::numeric_limits<float>::epsilon()) {
            throw std::runtime_error(
                    ...
                    " i = " + std::to_string(i) +
                    ", a = " + std::to_string(as[i]) +
                    ", b = " + std::to_string(bs[i]) +
                    ", expected sum = " + std::to_string(sum) +
    ...
    for (auto n : {1U * 1000 * 1000, 100U * 1000 * 1000}) {
        for (int platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
            ...
            for (int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
                main_for_device(platform, devices[deviceIndex], n);
                std::cout << std::endl;
            }
        }
    }

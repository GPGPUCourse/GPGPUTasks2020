...
std::pair<cl_platform_id, cl_device_id> chooseDevice() {
    ...
    return std::make_pair(bestPlatform, bestDevice);
}
...
{    
    ...
    auto platformDevice = chooseDevice();
    cl_platform_id platform = platformDevice.first;
    cl_device_id device = platformDevice.second;
    ...
    std::vector<cl_context_properties> properties{CL_CONTEXT_PLATFORM, (cl_context_properties)platform, 0};
    cl_context context = clCreateContext(properties.data(), 1, &device, nullptr, nullptr, &errorCode);
    ...
    unsigned long buffer_size = sizeof(float) * n;

    cl_mem aBuffer = clCreateBuffer(context, CL_MEM_READ_ONLY, buffer_size, nullptr, &errorCode);
    OCL_SAFE_CALL(errorCode);

    cl_mem bBuffer = clCreateBuffer(context, CL_MEM_READ_ONLY, buffer_size, nullptr, &errorCode);
    OCL_SAFE_CALL(errorCode);

    cl_mem cBuffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY, buffer_size, nullptr, &errorCode);
    OCL_SAFE_CALL(errorCode);

    OCL_SAFE_CALL(clEnqueueWriteBuffer(commandQueue, aBuffer, CL_TRUE, 0, buffer_size, as.data(), 0, nullptr, nullptr));
    OCL_SAFE_CALL(clEnqueueWriteBuffer(commandQueue, bBuffer, CL_TRUE, 0, buffer_size, bs.data(), 0, nullptr, nullptr));
    ...
    cl_program program = clCreateProgramWithSource(context, 1, &sources_ptr, nullptr, &errorCode);
    OCL_SAFE_CALL(errorCode);
    
    // TODO 8 Теперь скомпилируйте программу и напечатайте в консоль лог компиляции
    // см. clBuildProgram
    errorCode = clBuildProgram(program, 1, &device, "-w -Werror", nullptr, nullptr);
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        timer t;
        for (unsigned int i = 0; i < 20; ++i) {
            OCL_SAFE_CALL(clEnqueueReadBuffer(commandQueue, cBuffer, CL_TRUE, 0, buffer_size, cs.data(), 0, nullptr, nullptr));
            t.nextLap();
        }

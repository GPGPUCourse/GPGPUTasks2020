...
std::vector<cl_platform_id> getAllPlatforms()
{
...
std::vector<cl_device_id> getAllDevices(cl_platform_id platform)
{
...
std::vector<unsigned char> getPlatformStringInfo(cl_platform_id platform, cl_platform_info info)
{
    size_t platformInfoSize = 0;
    OCL_SAFE_CALL(clGetPlatformInfo(platform, info, 0, nullptr, &platformInfoSize));
    std::vector<unsigned char> platformInfo(platformInfoSize, 0);
    OCL_SAFE_CALL(clGetPlatformInfo(platform, info, platformInfoSize, platformInfo.data(), nullptr));
    return platformInfo;
}

std::vector<unsigned char> getDeviceStringInfo(cl_device_id device, cl_platform_info info)
{
...
int main()
{
    ...
    std::vector<cl_platform_id> platforms = getAllPlatforms();

    cl_platform_id platform = nullptr;
    bool gpuIsChosen = false;
    cl_device_id device = nullptr;;
    for (cl_platform_id curPlatform : platforms)
    {
        std::vector<cl_device_id> devices = getAllDevices(curPlatform);
        for (cl_device_id curDevice : devices)
        {
            cl_device_type deviceType = 0;
            OCL_SAFE_CALL(clGetDeviceInfo(curDevice, CL_DEVICE_TYPE, sizeof(cl_device_type), &deviceType, nullptr));
    ...
    if (device == nullptr)
        throw std::runtime_error("Device isn't chosen");

    std::cout << "Platform name: " << getPlatformStringInfo(platform, CL_PLATFORM_NAME).data() << std::endl;
    std::cout << "Device name: " << getDeviceStringInfo(device, CL_DEVICE_NAME).data() << std::endl;
    ...
    cl_command_queue commandQueue = clCreateCommandQueue(context, device, {}, &error);
    OCL_SAFE_CALL(error);
    ...
    cl_mem asGpu = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, as.data(), &error);
    OCL_SAFE_CALL(error);
    ...
    const char* string = kernel_sources.c_str();
    size_t stringSize = kernel_sources.size();
    cl_program program = clCreateProgramWithSource(context, 1, &string, &stringSize, &error);
    OCL_SAFE_CALL(error);
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        // - Среднее время выполнения кернела равно t.lapAvg() секунд
        std::cout << "VRAM bandwidth: " << ((3.0 * n * sizeof(float)) / (1024.0 * 1024 * 1024)) / t.lapAvg() << " GB/s" << std::endl;
    }
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            // clEnqueueReadBuffer...
            cl_event event;
            OCL_SAFE_CALL(clEnqueueReadBuffer(commandQueue, csGpu, CL_FALSE, 0, sizeof(float) * n, cs.data(), 0, nullptr, &event));
            OCL_SAFE_CALL(clWaitForEvents(1, &event));
            t.nextLap();
        }
        std::cout << "Result data transfer time: " << t.lapAvg() << "+-" << t.lapStd() << " s" << std::endl;
        std::cout << "VRAM -> RAM bandwidth: " << ((n * sizeof(float)) / (1024*1024*1024.0)) / t.lapAvg() << " GB/s" << std::endl;
    }

...
{    
    ...
    enum {
        ...
        CPU,
        GPU
    } chosenDeviceType = NONE;
    cl_platform_id chosenPlatformId;
    ...
    for (size_t platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
        ...
        for (size_t deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
            ...
            if (deviceType & CL_DEVICE_TYPE_CPU) {
                if (chosenDeviceType < CPU) {
                    chosenDeviceType = CPU;
                    chosenPlatformId = platformId;
                    chosenDeviceId = deviceId;
                }
            }
            if (deviceType & CL_DEVICE_TYPE_GPU) {
                if (chosenDeviceType < GPU) {
                    chosenDeviceType = GPU;
                    chosenPlatformId = platformId;
                    chosenDeviceId = deviceId;
                }
            }
    ...
    if (chosenDeviceType == NONE)
        throw std::runtime_error("No appropriate CL device was found");
    ...
    cl_context context = clCreateContext(nullptr,
        ...
        if (errinfo) {
            std::cerr << "Error occured while creating CL context: " << errinfo << std::endl;
        }
    }, nullptr, &errcode);
    ...
    cl_command_queue commandQueue = clCreateCommandQueue(context, chosenDeviceId,
                                                         0,
                                                         &errcode);
    OCL_SAFE_CALL(errcode);
    ...
    cl_mem gpuAs = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(as[0]) * as.size(), as.data(), &errcode);
    OCL_SAFE_CALL(errcode);
    cl_mem gpuBs = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(bs[0]) * bs.size(), bs.data(), &errcode);
    OCL_SAFE_CALL(errcode);
    cl_mem gpuCs = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(cs[0]) * cs.size(), nullptr, &errcode);
    OCL_SAFE_CALL(errcode);
    ...
    const char *ptr = kernel_sources.c_str();
    cl_program program = clCreateProgramWithSource(context, 1, &ptr, &length, &errcode);
    OCL_SAFE_CALL(errcode);

    // TODO 8 Теперь скомпилируйте программу и напечатайте в консоль лог компиляции
    // см. clBuildProgram

    errcode = clBuildProgram(program, 0, nullptr, "-Isrc/cl/", nullptr, nullptr);
    ...
    OCL_SAFE_CALL(clGetProgramBuildInfo(program, chosenDeviceId, CL_PROGRAM_BUILD_LOG, log_size, log.data(), nullptr));
    if (log_size > 1) {
    ...
    {
        unsigned int i = 0;
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(float*), &gpuAs));
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(float*), &gpuBs));
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(float*), &gpuCs));
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            cl_event event;
            OCL_SAFE_CALL(clEnqueueReadBuffer(commandQueue, gpuCs, CL_TRUE, 0, sizeof(float) * n, cs.data(), 0, nullptr, &event));
            t.nextLap();
        }
...

    {
        ...
        std::cout << "CPU GFlops: " << n / t.lapAvg() / 1e9 << std::endl;
    }

    clReleaseMemObject(gpuCs);
    clReleaseMemObject(gpuBs);
    clReleaseMemObject(gpuAs);

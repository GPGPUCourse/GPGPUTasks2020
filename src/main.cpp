...
cl_device_id getDevice() {
	// Для наглядности просто вырезал кусок из первой лабы (чтобы выводы остались)
	// Если есть видеокарты - выберем одну из них, если нет - то один из процессоров.
	cl_device_id choosenDevice = nullptr;
	bool gpuFound = false;
	...
	for (int platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
		...
		for (int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
			...
			case CL_DEVICE_TYPE_CPU:
				...
				if (!gpuFound) choosenDevice = deviceId;
				break;
			case CL_DEVICE_TYPE_GPU:
				std::cout << "\t\tDevice type: GPU" << std::endl;
				choosenDevice = deviceId;
				gpuFound = true;
				break;
			default:
	...
	if (choosenDevice == nullptr)
		throw std::runtime_error("No OpenCL devices found");
	
	std::cout << "\n\nChoosen device is " << getDeviceInfo(choosenDevice, CL_DEVICE_NAME).data() << std::endl;
...
int main() {    
	...
	// Согласно документации, если явно не указано CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE - то очередь in-order
	...
	cl_mem a_buf = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, n * sizeof(float), as.data(), &errorcode);
	OCL_SAFE_CALL(errorcode);

	cl_mem b_buf = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, n * sizeof(float), bs.data(), &errorcode);
	OCL_SAFE_CALL(errorcode);

	cl_mem c_buf = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_COPY_HOST_PTR, n * sizeof(float), cs.data(), &errorcode);
	OCL_SAFE_CALL(errorcode);
	...
	auto* src_ptr = kernel_sources.c_str();
	size_t len = 0; // Согласно документации наша строка воспримется как null-terminated, так что тут всё ок
	cl_program program = clCreateProgramWithSource(context, 1, &src_ptr, &len, &errorcode);
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        std::cout << "GFlops: " << n / (t.lapAvg() * 1000000000.0f) << std::endl;
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
			cl_event event;
			OCL_SAFE_CALL(clEnqueueReadBuffer(queue, c_buf, CL_TRUE, 0, n * sizeof(float), cs.data(), 0, nullptr, &event));
			OCL_SAFE_CALL(clWaitForEvents(1, &event));
			OCL_SAFE_CALL(clReleaseEvent(event));
            t.nextLap();
        }
	...
	// Убедился, поставив в кернеле a[i] - b[i]

...
{    
    ...
    cl_command_queue command_queue = clCreateCommandQueue(context, device, {}, &errcode);
    OCL_SAFE_CALL(errcode);
    ...
    auto asBuf = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, as.data(), &errcode);
    OCL_SAFE_CALL(errcode);
    auto bsBuf = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, bs.data(), &errcode);
    OCL_SAFE_CALL(errcode);
    auto csBuf = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, cs.data(), &errcode);
    OCL_SAFE_CALL(errcode);
    ...
    const auto* char_sting = kernel_sources.c_str();
    auto char_string_size = kernel_sources.size();
    auto program = clCreateProgramWithSource(context, 1, &char_sting, &char_string_size, &errcode);
    OCL_SAFE_CALL(errcode);
    ...
    {
        unsigned int i = 0;
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(cl_mem), &asBuf));
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(cl_mem), &bsBuf));
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(cl_mem), &csBuf));
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            ...
            // clWaitForEvents...
            cl_event cl_event;
            OCL_SAFE_CALL(clEnqueueNDRangeKernel(command_queue, kernel, 1, nullptr, &global_work_size, &workGroupSize, 0, nullptr, &cl_event));
            OCL_SAFE_CALL(clWaitForEvents(1, &cl_event));
        ...
        const size_t gflops = 1e09;
        std::cout << "GFlops: " << n / t.lapAvg() / gflops << std::endl;
        ...
        std::cout << "VRAM bandwidth: " << 3 * n  * sizeof(float) / t.lapAvg() / gbyte << " GB/s" << std::endl;
    }
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            cl_event cl_event;
            OCL_SAFE_CALL(clEnqueueReadBuffer(command_queue, csBuf, CL_FALSE, 0, sizeof(float) * n, cs.data(), 0, nullptr, &cl_event));
            OCL_SAFE_CALL(clWaitForEvents(1, &cl_event));
            t.nextLap();
        }
        std::cout << "Result data transfer time: " << t.lapAvg() << "+-" << t.lapStd() << " s" << std::endl;
        const size_t gbyte = 1024 * 1024 * 1024;
        std::cout << "VRAM -> RAM bandwidth: " << n * sizeof(float) / t.lapAvg() / gbyte << " GB/s" << std::endl;
    }

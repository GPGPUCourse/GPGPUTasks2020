...
int main()
{
    ...
    for (int platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
        ...
        try {
            ...
            std::cerr << "clGetPlatformInfo(239): " << e.what() << std::endl;
        }

        // TODO 1.2
        // Аналогично тому как был запрошен список идентификаторов всех платформ - так и с названием платформы, теперь, когда известна длина названия - его можно запросить:
        std::vector<unsigned char> platformName(platformNameSize, 0);
        OCL_SAFE_CALL(clGetPlatformInfo(platforms[0], CL_PLATFORM_NAME, platformNameSize, platformName.data(), nullptr));
        ...
        OCL_SAFE_CALL(clGetPlatformInfo(platforms[0], CL_PLATFORM_VENDOR, platformVendorSize, platformVendor.data(), nullptr));
        ...
        for (int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
            ...
            // didn't know how to uncode deviceType so pasted from https://gist.github.com/tzutalin/51a821f15a735024f16b
            ...
            size_t deviceGlobalMemorySizeSize = 0;
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_SIZE, 0, nullptr, &deviceGlobalMemorySizeSize));
            cl_ulong deviceGlobalMemorySize = 0;
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_SIZE, deviceGlobalMemorySizeSize, &deviceGlobalMemorySize, nullptr));
            std::cout << "    Global Memory size: " << (deviceGlobalMemorySize >> 20) << " Mb" << std::endl << std::endl;
            ...
            std::vector<unsigned char> deviceOpenCL(deviceOpenCLVersionSize, 0);
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_VERSION , deviceOpenCLVersionSize, deviceOpenCL.data(), nullptr));
            std::cout << "    Device OpenCL version: " << deviceOpenCL.data() << std::endl;
            ...
            std::vector<unsigned char> deviceDriver(deviceDriverVersionSize, 0);
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DRIVER_VERSION , deviceDriverVersionSize, deviceDriver.data(), nullptr));
            ...
            OCL_SAFE_CALL(clGetDeviceIDs(platforms[0], CL_DEVICE_MAX_COMPUTE_UNITS, 0, nullptr, &computeUnitsCount));
            std::cout << "    Number of compute units: " << computeUnitsCount << std::endl;
        }
    }

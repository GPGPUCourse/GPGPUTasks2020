...
cl_device_id getGpuOrCpuDeviceId() {
    cl_uint platformsCount = 0;
    OCL_SAFE_CALL(clGetPlatformIDs(0, nullptr, &platformsCount));

    std::vector <cl_platform_id> platforms(platformsCount);
    OCL_SAFE_CALL(clGetPlatformIDs(platformsCount, platforms.data(), nullptr));
    cl_device_id defaultIdCPU = nullptr;
    cl_device_id defaultIdGPU = nullptr;
    ...
    for (int platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
        ...
        OCL_SAFE_CALL(clGetPlatformInfo(platform, CL_PLATFORM_VENDOR,
                                        platformVendorSize, platformVendor.data(), nullptr));
        ...
        for (int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
            ...
            if (deviceType == CL_DEVICE_TYPE_CPU) {
                ...
                deviceTypeString = "smth else";
            }

        }
    }
    if (defaultIdGPU == nullptr) {
        return defaultIdCPU;
    }
    return defaultIdGPU;
}
...
int main() {
    ...
    cl_device_id deviceId = getGpuOrCpuDeviceId();
    ...
    cl_mem a_gpu = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, as.data(),
                                  &errcode);
    OCL_SAFE_CALL(errcode);

    cl_mem b_gpu = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, bs.data(),
                                  &errcode);
    OCL_SAFE_CALL(errcode);

    cl_mem c_gpu = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, cs.data(),
                                  &errcode);
    OCL_SAFE_CALL(errcode);
    ...
    cl_program program = clCreateProgramWithSource(context, 1, &str_ptr, nullptr, &errcode);
    OCL_SAFE_CALL(errcode);
    ...
    if (errcode == CL_BUILD_PROGRAM_FAILURE) {
        ...
        if (log_size > 1) {
            OCL_SAFE_CALL(clGetProgramBuildInfo(program, deviceId, CL_PROGRAM_BUILD_LOG,
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        size_t global_work_size = (n + workGroupSize - 1) / workGroupSize * workGroupSize; //// = roundedUpN -- ??
        timer t; // Это вспомогательный секундомер, он замеряет время своего создания и позволяет усреднять время нескольких замеров
        for (unsigned int i = 0; i < 20; ++i) {
            cl_event event; //// = clCreateUserEvent(context, &errcode); -- does not work
        ...
        std::cout << "GFlops: " << (n * 1.0 / t.lapAvg() / 1e9) << std::endl;
        ...
        // - Среднее время выполнения кернела равно t.lapAvg() секунд
        std::cout << "VRAM bandwidth: " << 3.0 * n * sizeof(float) / t.lapAvg() / (1024 * 1024 * 1024) << " GB/s"
                  << std::endl;
    }
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            cl_event event;
            OCL_SAFE_CALL(clEnqueueReadBuffer(commandQueue, c_gpu, CL_TRUE, 0, n * sizeof(float),
        ...
        std::cout << "VRAM -> RAM bandwidth: " << 1.0 * n * sizeof(float) / t.lapAvg() / (1024 * 1024 * 1024) << " GB/s"
                  << std::endl;
    }

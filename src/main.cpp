...
{    
    ...
    cl_device_type pickedDeviceType = 0;
    ...
    for (auto platform: platforms) {
        ...
        for (auto device: devices) {
            ...
            if (pickedDeviceType == 0 || (pickedDeviceType != CL_DEVICE_TYPE_GPU && deviceType == CL_DEVICE_TYPE_GPU)){
                pickedDevice = device;
                pickedDeviceType = deviceType;
            }
        }
    }

    size_t deviceNameSize = 0;
    OCL_SAFE_CALL(clGetDeviceInfo(pickedDevice, CL_DEVICE_NAME, 0, nullptr, &deviceNameSize));
    std::vector<char> deviceName(deviceNameSize, 0);
    OCL_SAFE_CALL(clGetDeviceInfo(pickedDevice, CL_DEVICE_NAME, deviceNameSize, deviceName.data(), nullptr));
    ...
    cl_int err_ret;
    cl_context context = clCreateContext(nullptr, 1, &pickedDevice, nullptr, nullptr, &err_ret);
    ...
    auto cqueue = clCreateCommandQueue(context, pickedDevice, 0, &err_ret);
    OCL_SAFE_CALL(err_ret);
    ...
    cl_mem a_buf = clCreateBuffer(context,
                                  CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                  as.size() * sizeof(float),
                                  as.data(),
                                  &err_ret);
    OCL_SAFE_CALL(err_ret);
    cl_mem b_buf = clCreateBuffer(context,
                                  CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                  bs.size() * sizeof(float),
                                  bs.data(),
                                  &err_ret);
    OCL_SAFE_CALL(err_ret);
...
//    OCL_SAFE_CALL(clBuildProgram(program, 1, &pickedDevice, nullptr, nullptr, nullptr));
    err_ret = clBuildProgram(program, 1, &pickedDevice, nullptr, nullptr, nullptr);
    ...
    cl_kernel kernel = clCreateKernel(program, "aplusb", &err_ret);
    OCL_SAFE_CALL(err_ret);

    // TODO 10 Выставите все аргументы в кернеле через clSetKernelArg (as_gpu, bs_gpu, cs_gpu и число значений, убедитесь что тип количества элементов такой же в кернеле)
    {
         unsigned int i = 0;
         OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(float*), &a_buf));
         OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(float*), &b_buf));
         OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(float*), &c_buf));
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            ...
            // clWaitForEvents...
            cl_event event;
            OCL_SAFE_CALL(clEnqueueNDRangeKernel(cqueue,
                                                      kernel,
                                                      1,
                                                      nullptr,
                                                      &global_work_size,
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            // clEnqueueReadBuffer...
            cl_event event;
            OCL_SAFE_CALL(clEnqueueReadBuffer(cqueue,
                                                   c_buf,
                                                   CL_TRUE,
                                                   0,
                                                   n*sizeof(float),
    ...
    for (unsigned int i = 0; i < n; ++i) {
        if (cs[i] != as[i] + bs[i]) {
            throw std::runtime_error("CPU and GPU results differ!" + to_string(i));
        }
    }
    ...
    OCL_SAFE_CALL(clReleaseCommandQueue(cqueue));

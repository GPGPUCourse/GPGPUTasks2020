...
////        cl_device_id device = devices[deviceIndex];
    ...
    cl_int err = 0;
    cl_context context = clCreateContext(nullptr, /* devicesCount */1, &device, nullptr, nullptr, &err);
    OCL_SAFE_CALL(err);
    ...
    cl_command_queue queue = clCreateCommandQueue(context, device, /* in-order */0, &err);
    OCL_SAFE_CALL(err);
    ...
    cl_mem as_buf = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, n * sizeof(float), as.data(), &err);
    OCL_SAFE_CALL(err);
    cl_mem bs_buf = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, n * sizeof(float), bs.data(), &err);
    OCL_SAFE_CALL(err);
    cl_mem cs_buf = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_COPY_HOST_PTR, n * sizeof(float), cs.data(), &err);
    OCL_SAFE_CALL(err);
...
////    std::vector<std::string> kernel_sources; for multiple sources?? or std::string + std::string??
    {
    ...
    const char* strings = kernel_sources.data();
////    If lengths is NULL, all strings in the strings argument are considered null-terminated.
    cl_program program = clCreateProgramWithSource(context, /* kernel_sources.size() ???*/ 1, &strings, nullptr, &err);
    OCL_SAFE_CALL(err);

    // TODO 8 Теперь скомпилируйте программу и напечатайте в консоль лог компиляции
    // см. clBuildProgram
    OCL_SAFE_CALL(clBuildProgram(program, devicesCount, &device, "", nullptr, nullptr));
    ...
    //// 1e9 throws 9 code. Took 1e8 instead
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            cl_event event; // or std::vector<cl_event> event_list;??
            OCL_SAFE_CALL(clEnqueueNDRangeKernel(queue, kernel, /* WI dimension */1, /* offset */nullptr, &global_work_size, &workGroupSize, 0, nullptr, &event));
            OCL_SAFE_CALL(clWaitForEvents(/* num events */1, &event));
        ...
        std::cout << "\nKernel average time: " << t.lapAvg() << "+-" << t.lapStd() << " s" << std::endl;
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            cl_event event;
            err = clEnqueueReadBuffer(queue, cs_buf, CL_TRUE, 0, n * sizeof(float), cs.data(), 0, nullptr, &event);
            clWaitForEvents(1, &event);
            t.nextLap();
        }
        std::cout << "Result data transfer time: " << t.lapAvg() << "+-" << t.lapStd() << " s" << std::endl;
        std::cout << "VRAM -> RAM bandwidth: " << 1. * n * sizeof(float) / (1 << 30) / t.lapAvg() << " GB/s" << std::endl;
    }

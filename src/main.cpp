...
int main()
{
    ...
    for (int platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
        ...
        for (cl_device_id device: devices) {
			...
			std::cout << "\t\tDevice memory size (MB): " << deviceMemSize / (1 << 20) << std::endl;
	
			cl_ulong deviceCacheSize;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_CACHE_SIZE, sizeof(cl_ulong), &deviceCacheSize, nullptr));
			std::cout << "\t\tDevice memory cache size (KB): " << deviceCacheSize / (1 << 10) << std::endl;

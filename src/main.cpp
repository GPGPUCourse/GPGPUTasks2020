...
int main()
{
    ...
    std::cout << "Device selected: " << deviceName.data() << std::endl;
    ...
    //cl_context_properties properties[2] = {CL_CONTEXT_PLATFORM, 0};
    ...
    unsigned int n = 50*1000*1000;///*100
    /// *100 -> what(): OpenCL error code -6 encountered at src\main.cpp:115 (CL_OUT_OF_HOST_MEMORY)
    ...
    ///cl_mem as_mem = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, n*sizeof(float), as.data(), &errcode_ret);
    ///OCL_SAFE_CALL(errcode_ret);
    ///cl_mem bs_mem = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, n*sizeof(float), bs.data(), &errcode_ret);
    ///OCL_SAFE_CALL(errcode_ret);
    ///cl_mem cs_mem = clCreateBuffer(context, CL_MEM_WRITE_ONLY, n*sizeof(float), nullptr, &errcode_ret);
    ///OCL_SAFE_CALL(errcode_ret);

    cl_mem as_mem = clCreateBuffer(context, CL_MEM_READ_ONLY, n*sizeof(float), nullptr, &errcode_ret);
    OCL_SAFE_CALL(errcode_ret);
    cl_mem bs_mem = clCreateBuffer(context, CL_MEM_READ_ONLY, n*sizeof(float), nullptr, &errcode_ret);
    OCL_SAFE_CALL(errcode_ret);
    cl_mem cs_mem = clCreateBuffer(context, CL_MEM_WRITE_ONLY, n*sizeof(float), nullptr, &errcode_ret);
    OCL_SAFE_CALL(errcode_ret);

    OCL_SAFE_CALL(clEnqueueWriteBuffer(queue, as_mem, CL_TRUE, 0, n*sizeof(float), as.data(), 0, nullptr, nullptr));
    OCL_SAFE_CALL(clEnqueueWriteBuffer(queue, bs_mem, CL_TRUE, 0, n*sizeof(float), bs.data(), 0, nullptr, nullptr));
    ...
    cl_program program = clCreateProgramWithSource(context, 1, &source, &source_size, &errcode_ret);
    OCL_SAFE_CALL(errcode_ret);
    ...
    {
        unsigned int i = 0;
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(cl_mem), &as_mem));
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(cl_mem), &bs_mem));
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(cl_mem), &cs_mem));
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        cl_event event = 0;///clCreateUserEvent(context, &errcode_ret);
        OCL_SAFE_CALL(errcode_ret);
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i)
        {
            OCL_SAFE_CALL(clEnqueueReadBuffer(queue, cs_mem, CL_FALSE, 0, n*sizeof(float), cs.data(), 0, nullptr, &event));
            OCL_SAFE_CALL(clWaitForEvents(1, &event));
            t.nextLap();
        }
        ///OCL_SAFE_CALL(clReleaseEvent(event));
    ...
    for (unsigned int i = 0; i < n; ++i)
    {
        if (fabs(cs[i]-as[i]-bs[i])>eps)///(cs[i] != as[i] + bs[i])
        {
            std::cout << i << " : " << std::setprecision(8) << fabs(cs[i]-as[i]-bs[i]) << std::endl;

...
{    
    ...
    std::cout << "Command queue Released" << std::endl;
    ...
    cl_mem buf_as = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, as.data(), &err);
    cl_mem buf_bs = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, bs.data(), &err);
    cl_mem buf_cs = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(float) * n, cs.data(), &err);
    ...
    const size_t source_size = kernel_sources.size();
    ...
    {
        unsigned int i = 0;
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(cl_mem), (void*)&buf_as));
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(cl_mem), (void*)&buf_bs));
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(cl_mem), (void*)&buf_cs));
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(unsigned int), (void *)&n));
    }
...

    {
        timer t;
        for (unsigned int i = 0; i < 20; ++i) {
            OCL_SAFE_CALL(clEnqueueReadBuffer(queue, buf_cs, CL_TRUE, 0, sizeof(float) * n, cs.data(), 0, NULL, &event));
            t.nextLap();
        }

...
int main(int argc, char **argv)
{
...

    {
        ...
        for (int iter = 0; iter < benchmarkingIters; ++iter) {
            // set-up 2D NDRange
            const unsigned int work_group_size_x = 16;
            const unsigned int global_work_size_x = (K + work_group_size_x - 1) / work_group_size_x * work_group_size_x;
            const unsigned int work_group_size_y = 16;
            const unsigned int global_work_size_y = (M + work_group_size_y - 1) / work_group_size_y * work_group_size_y;

            matrix_multiplication_kernel.exec(gpu::WorkSize(work_group_size_x, work_group_size_y,

...
//    float sizeX = 2.0f;
   ...
   context.activate();
   {
       ...
       gpu::gpu_mem_32f resImg;
       resImg.resizeN(width * height);

       gpu::WorkSize workSize(16, 8, width, height);
       timer t;
       for(int i = 0;i < benchmarkingIters;++i){
            kernel.exec(workSize,resImg,width,height,centralX - sizeX / 2.0f, centralY - sizeY / 2.0f,
                          sizeX, sizeY,iterationsLimit,0);


            resImg.readN(gpu_results.ptr(), width * height);
        ...
        std::cout << "GPU iterations fraction: " << 100.0 * realIterationsFraction / (width * height) << "%" << std::endl;

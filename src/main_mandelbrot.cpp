...
//    float sizeX = 2.0f;
    ...
    gpu_res.resizeN(width * height * 1);
    {
        ...
        for (int i = 0; i < benchmarkingIters; ++i) {
          // 8*8 = 64 = 2 * 32
          kernel.exec(gpu::WorkSize(8, 8, width, height), gpu_res, height,
                      width, centralX - sizeX / 2.0f, centralY - sizeY / 2.0f,
                      sizeX, sizeY, iterationsLimit, 0);
          t.nextLap();
        }


        gpu_res.readN(gpu_results.ptr(), height * width);

...
//    float sizeX = 2.0f;
    ...
    images::Image<unsigned char> image_cpu(width, height, 3);
    images::Image<unsigned char> image_gpu(width, height, 3);
...

    {
        ...
        renderToColor(cpu_results.ptr(), image_cpu.ptr(), width, height);
        image_cpu.savePNG("mandelbrot_cpu.png");
    }
    ...
    context.activate();
    {
        ...
        //// buffer
        gpu::gpu_mem_32f img_buf;
        img_buf.resizeN(width * height);
        //// workgroup
        unsigned int workGroupSize_x = 8;
        unsigned int workGroupSize_y = 8;
        unsigned int global_work_size_x = (width + workGroupSize_x - 1) / workGroupSize_x * workGroupSize_x;
        unsigned int global_work_size_y = (height + workGroupSize_y - 1) / workGroupSize_y * workGroupSize_y;
        //// execution
        timer t;
        for (int i = 0; i < benchmarkingIters; ++i) {
            kernel.exec(gpu::WorkSize(workGroupSize_x, workGroupSize_y, global_work_size_x, global_work_size_y),
                        img_buf,
                        width, height,
                        centralX - sizeX / 2.0f, centralY - sizeY / 2.0f,
                        sizeX, sizeY,
        ...
        img_buf.readN(gpu_results.ptr(), width * height);
        ...
        renderToColor(gpu_results.ptr(), image_gpu.ptr(), width, height);
        image_gpu.savePNG("mandelbrot_gpu.png");
    }
    //// difference
    {
        double errorAvg = 0.0;

...
void printInfo(unsigned int width, unsigned int height,
                images::Image<float>& results, 
                unsigned int iterations, const timer& t, const std::string& device)
{
    size_t flopsInLoop = 10;
    size_t maxApproximateFlops = width * height * iterations * flopsInLoop;
    size_t gflops = 1000*1000*1000;
    std::cout << device + ": " << t.lapAvg() << "+-" << t.lapStd() << " s" << std::endl;
    std::cout << device + ": " << maxApproximateFlops / gflops / t.lapAvg() << " GFlops" << std::endl;

    double realIterationsFraction = 0.0;
    for (int j = 0; j < height; ++j) {
        for (int i = 0; i < width; ++i) {
            realIterationsFraction += results.ptr()[j * width + i];
        }
    }
...
int main(int argc, char **argv)
{
    ...
    unsigned int benchmarkingIters = 1;
...
//    float sizeX = 2.0f;
...

    {
        ...
        printInfo(width, height, cpu_results, iterationsLimit, t, "CPU");
   ...
   context.activate();
   {
       ...
       // обратите внимание что и произвдительность относительно референсной ЦПУ реализации выросла почти в восемь раз
        ...
        printInfo(width, height, gpu_results, iterationsLimit, t, "GPU");

...
//    float sizeX = 2.0f;
    ...
    context.activate();
    {
        ...
        timer tick;
        gpu::gpu_mem_32f vram_buffer;
        vram_buffer.resizeN(width * height);
        ...
        vram_buffer.readN(gpu_results.ptr(), width * height);

        size_t flopsInLoop = 10;
        size_t maxApproximateFlops = width * height * iterationsLimit * flopsInLoop;
        size_t gflops = 1000*1000*1000;
        std::cout << "GPU: " << tick.lapAvg() << "+-" << tick.lapStd() << " s" << std::endl;
        std::cout << "GPU: " << maxApproximateFlops / gflops / tick.lapAvg() << " GFlops" << std::endl;

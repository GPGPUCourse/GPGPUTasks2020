...
int main(int argc, char **argv)
{
    ...
    gpu::gpu_mem_32f as_gpu1;
    gpu::gpu_mem_32f as_gpu2;
    as_gpu1.resizeN(n);
    as_gpu2.resizeN(n);

    gpu::gpu_mem_32f *Src = &as_gpu1;
    gpu::gpu_mem_32f *Res = &as_gpu2;
    {
        ...
        for (int iter = 0; iter < benchmarkingIters; ++iter) {
            ...
            for (unsigned int CurrentSize = 1; CurrentSize < n; CurrentSize <<= 1)
            {
              merge.exec(gpu::WorkSize(workGroupSize, global_work_size),
                         *Src, *Res, n, CurrentSize);
              std::swap(Src, Res);
            }

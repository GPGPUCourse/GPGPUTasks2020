...
int main(int argc, char **argv)
{
    ...
    as_gpu_out.resizeN(n);
    {
        ...
        for (int iter = 0; iter < benchmarkingIters; ++iter) {
            ...
            for (unsigned int sorted_size = 1; sorted_size < n; sorted_size *= 2) {
                merge.exec(gpu::WorkSize(workGroupSize, global_work_size),
                           as_gpu, as_gpu_out, sorted_size, n);
                // swap to avoid rewriting data
                as_gpu.swap(as_gpu_out);
            }
            t.nextLap();
        }

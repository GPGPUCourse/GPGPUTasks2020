...
int main(int argc, char **argv) {
    ...
    gpu::gpu_mem_32f as_gpu, as_gpu_out;
    as_gpu.resizeN(n);
    as_gpu_out.resizeN(n);
    {
        ...
        for (int iter = 0; iter < benchmarkingIters; ++iter) {
            ...
            for (int power2 = 1; power2 < n; power2 *= 2) {
//                std::cout << "SWAP                " << power2 << std::endl;
                merge.exec(gpu::WorkSize(workGroupSize, global_work_size),
                           as_gpu, as_gpu_out, power2, n);
    ...
    for (int i = 0; i < n; ++i) {
        if (as[i] != cpu_sorted[i]) {
            std::cout << i << " out of " << n << " " << as[i] << "!=" << cpu_sorted[i] << std::endl;
        }

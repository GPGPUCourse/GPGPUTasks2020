...
int main(int argc, char **argv) {
...

  {
    ...
    for (int iter = 0; iter < benchmarkingIters; ++iter) {
      // TODO
      unsigned int whs = 16;
      unsigned int gws_x = ceil(double(M) / whs) * whs;
      unsigned int gws_y = ceil(double(K) / whs) * whs;

      matrix_transpose_kernel.exec(gpu::WorkSize(whs, whs, gws_x, gws_y),
                                   as_gpu, as_t_gpu, M, K);
    ...
    std::cout << "GPU: " << t.lapAvg() << "+-" << t.lapStd() << " s"
              << std::endl;
    std::cout << "GPU: " << M * K / 1000.0 / 1000.0 / t.lapAvg()
              << " millions/s" << std::endl;
  }
